...
int dump_info_for_fuzz(struct f2fs_sb_info *sbi)
{

    u64 blk_addr;

    for (blk_addr = SM_I(sbi)->main_blkaddr;
        blk_addr < le64_to_cpu(F2FS_RAW_SUPER(sbi)->block_count);
        blk_addr++) {
